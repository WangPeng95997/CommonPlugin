...
namespace CommonPlugin
{
        ...
        IEventHandlerWaitingForPlayers, IEventHandlerWarheadChangeLever, IEventHandlerWarheadStopCountdown, IEventHandlerWarheadDetonate, IEventHandlerSetInventory, IEventHandlerCassieTeamAnnouncement
    {
        ...
        const int healCooldown = 3;
        const int maxPlayer = 25;
        const float medkitHeal = 70.0f;
        const float medkitHeal2 = 12.5f;
        const int powercutCooldown = 30;
        const int scp079switchTime = 30;

        const float scpHeal = 10.0f;
        const float traumaDamage = 0.25f;
        const float scp035Hit = 5.0f;
        const float scp035Heal = 35.0f;
        const float scp035Heal2 = 7.0f;
        const float scp096Heal = 5.0f;
        const float scp106Kill = 20.0f;
        const float scp682Heal = 8.0f;
        const float scp682Kill = 40.0f;
        ...
        double warheadRate;
        bool warheadSystem;
        Broadcast personalBC;
        Scp079Level scp079Level = Scp079Level.Level_1;
        ...
        public static List<ushort> TrapItems { get; private set; } = new List<ushort>();
        ...
        private IEnumerator<float> Timing_SelfHeal(ReferenceHub hub, HealthController healthController, RoleType roleType)
        {
            int n = 0;
        ...
        private IEnumerator<float> Timing_OnSetRole(ReferenceHub hub)
        {
            ...
            switch (hub.characterClassManager.NetworkCurClass)
            {
                ...
                case RoleType.Scp049:
                    ...
                    Timing.RunCoroutine(Timing_SelfHeal(hub, healthControler, hub.characterClassManager.NetworkCurClass));
                    break;
